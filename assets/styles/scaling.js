...
const isSmall = width <= 320 && !Constants.platform?.isPad;
...
const guidelineBaseWidth = () => {
  ...
  return 360; // if device is not small, return 350
};

// Define the guideline base height based on the device's width
const guidelineBaseHeight = () => {
  if (isSmall) {
    // if device is small, return 320 (enough vertical space without taking up too much screen real estate)
    return 320;
  } else if (width > 410) {
